...
NomeRepFile = "transaction history 233348.csv"
CexName = "Bitmex"
skiprows = 0
...
while idx < len(df):
    ...
    if row.transactType == 'SpotTrade' or row.transactType == 'Conversion' or row.transactType == 'AutoConversion':
        ...
        if row.transactType == 'Transfer' and currency1 == 'BMEX' and row.timestamp < con_lib.to_timestamp('2022-11-11 04:49:15'): 
            event['asset_b'] = 'EUR'
            print(row.timestamp , 'BMEx valore ancoras a ZERO' , datetime.utcfromtimestamp(row.timestamp))
